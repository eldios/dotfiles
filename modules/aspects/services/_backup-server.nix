{
  ...
  # must not be able to read) so the server can decrypt it. Rotating a client
  # key is a single-file edit to borg.yaml.
  clients = ["lele8845ace" "lele9iyoga"];

  secretFor = host: "borg/clients/${host}/pubkey";
  serveCmd = host: ''command="${pkgs.borgbackup}/bin/borg serve --restrict-to-path ${borgRoot}/${host}",restrict'';
in {
  users.groups.borg = {};
  ...
  users.users.borg = {
    ...
    description = "borg backup repo owner (restricted ssh, see the backup-server aspect)";
  };
  ...
  sops.templates."borg-authorized-keys" = {
    ...
    content =
      lib.concatStringsSep "\n" (map (
          host: "${serveCmd host} ${config.sops.placeholder.${secretFor host}}"
        )
        clients)
      + "\n";
  ...
  # /data is a post-boot LUKS-on-mdadm mount (see the host's data-storage
  # module), so systemd-tmpfiles runs too early. Create/fix the repo dirs
  # after the mount is up.
