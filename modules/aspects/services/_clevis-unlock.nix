...
#
# Generic: resolves the JWE by hostname and takes the root LUKS mapper name
# from the host's `luksRoot` declaration in hosts.nix. Self-activates only
# once the JWE exists in the secrets input (no-op before
...
# keyslots (crypttabExtraOpts below) and the passphrase (keyslot 0) remain.
{
  ...
  jweFile = "${inputs.secrets}/clevis/${host.name}.jwe";
  # Root LUKS mapper name, declared per host rather than read from
  # boot.initrd.luks.devices: this module writes crypttabExtraOpts into that
  # same option, and reading it back would recurse.
  luksName = host.luksRoot;
